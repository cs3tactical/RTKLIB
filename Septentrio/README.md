...
### Compile str2str using Docker (Debian x64)

If you want to build `str2str` in a clean Debian x64 environment using Docker, you can use the provided PowerShell script:

1. Make sure [Docker Desktop](https://docs.docker.com/desktop/) is installed and running on Windows.
2. Run the build script from the project root in PowerShell:

    ```powershell
    ./build_str2str_docker.ps1
    ```

This will:
- Build a Docker image based on Debian,
- Install all required build tools,
- Compile `str2str` from the source,
- Copy the resulting binary to `./str2str_docker_build.exe` in your project directory.

You can then use `./str2str_docker_build.exe` as your compiled binary.
